...
export default function RiskCaseDetailPage() {
  ...
  const caseId = (params.caseId as string) || "RC-001";
  ...
  useEffect(() => {
    async function load() {
      setLoading(true);
      try {
        const rc = await getRiskCaseById(caseId);
        setRiskCase(rc);
  ...
  if (!riskCase) {
    return (
      <div className="p-12 text-center space-y-4">
        ...
        <p className="text-xs text-muted-foreground">The requested case {caseId} does not exist.</p>
